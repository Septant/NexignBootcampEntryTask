...
public class UtilFuncs
{
    ...
    public static String dateFormat(int[] dateArr)
    {
        ...
        {
            formatted += dateArr[0];
            formatted += ("-" + decFormat(dateArr[1]) + "-" + decFormat(dateArr[2]) + " ");
            formatted += (decFormat(dateArr[3]) + ":" + decFormat(dateArr[4]) + ":" + decFormat(dateArr[5]));
        }

        else
        {
            formatted += (decFormat(dateArr[0]) + ":" + decFormat(dateArr[1]) + ":" + decFormat(dateArr[2]));
        }
        return formatted;
    }

    /**
     * Reads data in format of the following string:"\d\d, [\d]{11}, [\d]{14}, [\d]{14}, \d\d",
    ...
    /**
     * Prints reports for all numbers in incoming data.
     *
    ...
    {
        String report = reportRootPath + "report_for_" + num + ".txt";
        ...
        try
        {
            ...
            writer.write("Tariff Index: " + filteredByNumber.cdrList.get(0).getTariff().getType() + "\n");
            writer.write("----------------------------------------------------------------------------\n");
            ...
            for (CDR cdr : filteredByNumber.cdrList)
                writer.write("|     " + cdr.getCallType()
                              + "     | " + dateFormat(cdr.getCallStart())
                                  + " | " + dateFormat(cdr.getCallEnd())
                                  + " | " + dateFormat(cdr.getCallDuration())
